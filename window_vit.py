...
base_dir = 'D:/Jiwoon/dataset/window/original/total/'
classes = ['bad', 'good']
...
def plot_metrics(history, metric, filepath):
    ...
    plt.grid(True)
    plt.savefig(os.path.join(filepath, f"convbase_{metric}.png"))
    plt.close()
...
def plot_roc_auc(y_true, y_pred, filepath):
    ...
    # plt.grid(True)
    plt.savefig(os.path.join(filepath, "convbase_roc_auc_curve.png"))
    plt.close()

def run_experiment():
    filepath = "./tmp/window_video_classifier"
    if not os.path.exists(filepath):
        os.makedirs(filepath)
    checkpoint = keras.callbacks.ModelCheckpoint(
        filepath+'/convbase_transformer_window.h5', monitor="val_acc", save_weights_only=True, save_best_only=True, verbose=1, mode='max'
    )
    ...
    model.load_weights(filepath+'/convbase_transformer_window.h5')
